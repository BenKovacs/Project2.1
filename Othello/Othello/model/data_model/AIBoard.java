...
public class AIBoard {
	...
	/**
	 * - check valid moves for that player - for each valid move, flip the disc
	 * and develop the next state
	 */
	public ArrayList<Point3D> getValidM() {
		...
		for (int x = 0; x < board.length; x++) {
			for (int y = 0; y < board[0].length; y++) {
				...
				if (flips > 0)
					validMoves.add(new Point3D(x, y, stabilityHeuristic(x, y) + flips));
	...
	private int heuristicPosition(int x, int y) {
		// heuristic that gives amount based on the position of the flipped
		// disks on the table
		if (x >= 3 && x <= 4 && y >= 3 && y <= 4)
			return 1;
		else {
