...
public class MainApp{
	...
	private void initialize() {
		...
		if (settings.getPlayer1().equalsIgnoreCase("human")) {
            ...
            playerList[0] = ai;
            ai.start();
        } else if (settings.getPlayer1().equalsIgnoreCase("smcts2")) {
			int runtime = 3000; //min runtime in millisecs
			int iterations = 0; //min iterations
			SuperMonteCarloTreeSearch2 ai = new SuperMonteCarloTreeSearch2(boardPanel, turn, runtime, iterations);
			playerList[0] = ai;
			ai.start();
		}
		...
		if (settings.getPlayer2().equalsIgnoreCase("human")) {
            ...
            playerList[1] = ai;
            ai.start();
        } else if (settings.getPlayer2().equalsIgnoreCase("smcts2")) {
			int runtime = 3000; //min runtime in millisecs
			int iterations = 0; //min iterations
			SuperMonteCarloTreeSearch2 ai = new SuperMonteCarloTreeSearch2(boardPanel, turn, runtime, iterations);
			playerList[1] = ai;
		...
		if (playerCount > 2){
			...
			if (settings.getPlayer3().equalsIgnoreCase("human")) {
                ...
                playerList[2] = ai;
                ai.start();
            } else if (settings.getPlayer3().equalsIgnoreCase("smcts2")) {
				int runtime = 3000; //min runtime in millisecs
				int iterations = 0; //min iterations
				SuperMonteCarloTreeSearch2 ai = new SuperMonteCarloTreeSearch2(boardPanel, turn, runtime, iterations);
				playerList[2] = ai;
		...
		if (playerCount > 3){
			...
			if (settings.getPlayer4().equalsIgnoreCase("human")) {
                ...
                playerList[3] = ai;
                ai.start();
            } else if (settings.getPlayer4().equalsIgnoreCase("smcts2")) {
				int runtime = 3000; //min runtime in millisecs
				int iterations = 0; //min iterations
				SuperMonteCarloTreeSearch2 ai = new SuperMonteCarloTreeSearch2(boardPanel, turn, runtime, iterations);
				playerList[3] = ai;
